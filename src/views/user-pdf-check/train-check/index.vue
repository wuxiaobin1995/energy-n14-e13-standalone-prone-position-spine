<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-09 14:32:17
 * @LastEditTime: 2022-01-26 09:56:30
 * @Description : 管理员查看用户训练报告
-->
<template>
  <div class="user-pdf-check-train">
    <!-- 下拉菜单 -->
    <el-select
      class="select"
      v-model="type"
...
export default {
  ...
  data() {
    return {
      ...
      options: [
        {
          value: 'activity-improvement',
          label: '活动度改善训练'
        },
        {
          value: 'abdominal-respiration',
          label: '腹式呼吸训练'
        },
        {
          value: 'static',
  ...
  methods: {
    ...
    handleToPdf(index, row) {
      this.$router.push({
        path: `/layout/${this.type}-train-pdf`,
        query: {
          dataId: row.dataId,
          routerName: `/layout/user-pdf-check-train`
        }
      })
    },
