<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-03 11:34:46
 * @LastEditTime: 2022-03-03 20:16:10
 * @Description : 静态训练-测量页面
-->
<template>
  <div
    class="static-train-measure"
...
export default {
  ...
  data() {
    return {
      ...
      audioSrc: path.join(
        __static,
        `narrate/${this.$store.state.voiceBaseSrc}/15.mp3`
      ),
      ...
      audioTwoSrc: path.join(
        __static,
        `narrate/${this.$store.state.voiceBaseSrc}/16.mp3`
      ),
      audioThreeSrc: path.join(
        __static,
        `narrate/${this.$store.state.voiceBaseSrc}/17.mp3`
      ),
      ...
      /* 其他 */
      oneSrc: require('@/assets/img/train-mode/static-train/1.png'),
      twoSrc: require('@/assets/img/train-mode/static-train/2.png'),
      threeSrc: require('@/assets/img/train-mode/static-train/3.png'),
  ...
  methods: {
    ...
    handleToPdf() {
      this.$router.push({
        path: '/layout/static-train-pdf',
        query: {
          dataId: this.dataId, // 字符串类型
          routerName: '/layout/static-train-set'
        }
      })
    },
