<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-03 11:33:23
 * @LastEditTime: 2022-05-24 14:09:52
 * @Description : 静态训练-参数设置页面
-->
<template>
  <div class="static-train-set">
    <!-- 语音播放 -->
    ...
    <div class="des">
      ...
      <div class="content">
        <div class="content__item">训练目的：静态动作下，内核心稳定训练；</div>
        <div class="content__item">
          动作要求：首先将光标移动到绿色区域内，随后腰腹部持续收紧使光标不晃动，上下肢保持一个指定动作，过程中将肩部和臀部紧贴软垫，直至训练结束。
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      audioSrc: path.join(
        __static,
        `narrate/${this.$store.state.voiceBaseSrc}/14.mp3`
      ),
  ...
  methods: {
    ...
    handleStart() {
      this.$router.push({
        path: '/layout/static-train-measure',
        query: {
