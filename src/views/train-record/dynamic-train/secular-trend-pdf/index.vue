 ...
 * @LastEditTime: 2022-03-05 18:41:54
 * @Description : 动态训练-长期趋势报告
-->
<template>
  <div
    class="dynamic-train-secular-trend-pdf"
    v-loading.fullscreen.lock="fullscreenLoading"
  >
    <!-- PDF区域 -->
    <div id="pdf" class="pdf-wrapper">
      <div class="up">
        <div class="title">动态训练-长期趋势报告</div>
        <div class="logo-img">
...
export default {
  ...
  data() {
    return {
      ...
      completionArrayOne: [], // 动作1完成度数组
      completionArrayTwo: [], // 动作2完成度数组
      completionArrayThree: [] // 动作3完成度数组
    }
  },
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (data.status === 1) {
            /* 成功 */
            this.completionArrayOne = []
            this.completionArrayTwo = []
            this.completionArrayThree = []
            for (let i = 0; i < data.result.length; i++) {
              const element = data.result[i]
              if (element.type === 'dynamic-1') {
                this.completionArrayOne.push(element.completion)
              } else if (element.type === 'dynamic-2') {
                this.completionArrayTwo.push(element.completion)
              } else if (element.type === 'dynamic-3') {
                this.completionArrayThree.push(element.completion)
              }
            }
        ...
        .catch(err => {
          this.$confirm(
            `[动态训练-长期趋势报告环节] ${err}。请确保网络连接正常，然后重试！`,
            '网络请求错误',
            {
              type: 'error',
              center: true,
    ...
    initChart() {
      ...
      this.option = {
        ...
        series: [
          ...
          {
            name: '动作3完成度',
            data: this.completionArrayThree,
            color: 'blue',
            type: 'line',
            smooth: false,
            showSymbol: false
    ...
    handlePrint() {
      this.$htmlToPdf(
        'pdf',
        `动态训练-长期趋势报告 ${this.$moment().format('YYYY-MM-DD HH_mm_ss')}`
      )
    },
...
.dynamic-train-secular-trend-pdf {
  width: 100%;
  height: 100%;
