<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-08 10:52:20
 * @LastEditTime: 2022-01-26 09:54:26
 * @Description : 活动度改善训练-数据记录
-->
<template>
  <div class="activity-improvement-train-record">
    <!-- 表格 -->
    <el-table
      class="table"
      :data="showData"
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios
        ...
        .catch(err => {
          this.$confirm(
            `[活动度改善训练-数据记录环节] ${err}。请确保网络连接正常，然后重试！`,
            '网络请求错误',
            {
              type: 'error',
              center: true,
    ...
    handleToPdf(index, row) {
      this.$router.push({
        ...
        query: {
          dataId: row.dataId,
          routerName: '/layout/activity-improvement-train-record'
        }
      })
    },
    ...
    handleSecularTrend() {
      this.$router.push({
        path: '/layout/activity-improvement-train-secular-trend-pdf',
        query: {
          routerName: '/layout/activity-improvement-train-record'
        }
      })
...
.activity-improvement-train-record {
  width: 100%;
  height: 100%;
  padding: 20px 40px;
