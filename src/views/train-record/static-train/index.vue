<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-08 11:39:09
 * @LastEditTime: 2022-01-26 09:55:40
 * @Description : 静态训练-数据记录
-->
<template>
  <div class="static-train-record">
    <!-- 表格 -->
    <el-table
      class="table"
      :data="showData"
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios
        ...
        .catch(err => {
          this.$confirm(
            `[静态训练-数据记录环节] ${err}。请确保网络连接正常，然后重试！`,
            '网络请求错误',
            {
              type: 'error',
              center: true,
    ...
    handleToPdf(index, row) {
      this.$router.push({
        ...
        query: {
          dataId: row.dataId,
          routerName: '/layout/static-train-record'
        }
      })
    },
    ...
    handleSecularTrend() {
      this.$router.push({
        path: '/layout/static-train-secular-trend-pdf',
        query: {
          routerName: '/layout/static-train-record'
        }
      })
