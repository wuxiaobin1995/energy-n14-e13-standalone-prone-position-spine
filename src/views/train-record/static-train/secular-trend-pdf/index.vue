 ...
 * @LastEditTime: 2022-03-05 18:42:01
 * @Description : 静态训练-长期趋势报告
-->
<template>
  <div
    class="static-train-secular-trend-pdf"
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios
        ...
        .catch(err => {
          this.$confirm(
            `[静态训练-长期趋势报告环节] ${err}。请确保网络连接正常，然后重试！`,
            '网络请求错误',
            {
              type: 'error',
              center: true,
    ...
    handlePrint() {
      this.$htmlToPdf(
        'pdf',
        `静态训练-长期趋势报告 ${this.$moment().format('YYYY-MM-DD HH_mm_ss')}`
      )
    },
...
.static-train-secular-trend-pdf {
  width: 100%;
  height: 100%;
