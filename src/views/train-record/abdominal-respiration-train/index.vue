<!--
 * @Author      : Mr.bin
 * @Date        : 2021-12-08 11:27:34
 * @LastEditTime: 2022-01-26 09:54:02
 * @Description : 腹式呼吸训练-数据记录
-->
<template>
  <div class="abdominal-respiration-train-record">
    <!-- 表格 -->
    <el-table
      class="table"
      :data="showData"
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios
        ...
        .catch(err => {
          this.$confirm(
            `[腹式呼吸训练-数据记录环节] ${err}。请确保网络连接正常，然后重试！`,
            '网络请求错误',
            {
              type: 'error',
              center: true,
    ...
    handleToPdf(index, row) {
      this.$router.push({
        ...
        query: {
          dataId: row.dataId,
          routerName: '/layout/abdominal-respiration-train-record'
        }
      })
    },
      ...
      this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          this.$axios
            ...
            .catch(err => {
              this.$alert(
                `[腹式呼吸训练-删除数据环节] ${err}。请确保网络连接正常，刷新页面后重试！`,
                '网络请求错误',
                {
                  showClose: false,
                  confirmButtonText: '刷新页面',
    ...
    handleSecularTrend() {
      this.$router.push({
        path: '/layout/abdominal-respiration-train-secular-trend-pdf',
        query: {
          routerName: '/layout/abdominal-respiration-train-record'
        }
      })
...
.abdominal-respiration-train-record {
  width: 100%;
  height: 100%;
  padding: 20px 40px;
